# firewall
    list1 = ['add-service=dns', 'add-service=ssh', 'add-port=3000/tcp', 'add-masquerade']
    list1.each do|l|
...
h1.each do |p, ip|
      ...
      execute "hremain" do
      command "firewall-cmd --permanent --add-forward-port=port=#{p1}:proto=tcp:toport=#{p2}:toaddr=192.168.1.110 --zone=external"
      action :run
      end
      end
...
h2 = {"9080" => "192.168.1.15", "4000" => "192.168.1.66", "50180" => "192.168.1.46", "8443" => "192.168.1.38", "2222" => "192.168.1.15", "8484" => "192.168.1.38", "8888" => "192.168.1.38", "9999" => "192.168.1.38", "16000" => "192.168.1.72", "9090" => "192.168.1.15"}
   ...
   execute "h2" do
   command "firewall-cmd --permanent --direct --add-rule ipv4 nat PREROUTING 0 -p tcp --dport #{p} -d 49.248.27.91 -j DNAT --to-destination #{ip}:#{p}"
   action :run
   end
   end
...
h3.each do |p, ip|
  ...
  execute "h3" do
  command "firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -p tcp --dport #{p} -s 192.168.1.0/24 -d #{ip} -j SNAT --to-source 192.168.1.5"
  action :run
      end
      end
...
command "firewall-cmd --permanent --direct --add-rule ipv4 nat PREROUTING 0 -p tcp --dport #{p1} -d 49.248.27.91 -j DNAT --to-destination 192.168.1.110:#{p2}"
          action :run
          end
          end
...
hremainpost.each do |p1, p2|
  ...
  command "firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -p tcp --dport #{p1} -s 192.168.1.0/24 -d 192.168.1.110 -j SNAT --to-source 192.168.1.5"
  action :run
  end
  end
